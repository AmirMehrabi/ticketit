...
class TicketsController extends Controller
{
    ...
    public function data($complete = false)
    {
        ...
        $collection
            ...
            ->leftJoin('subscription_services', 'subscription_services.id', '=', 'ticketit.subscription_service_id')
            ->leftJoin('contacts', 'contacts.id', '=', 'subscription_services.contact_id')
            ->select([
                'ticketit.id',
                'ticketit.subject AS subject',
                'ticketit_statuses.name AS status',
                'subscription_services.subject AS customer_subject',
                'contacts.primary_phone as customer_primary_phone',
                'contacts.first_name as customer_first_phone',
                'contacts.last_name as customer_last_name',
    ...
    public function create()
    {
        ...
        foreach(\App\Modules\Subscription\Models\SubscriptionService::get() as $key => $customer) {
            if(!empty($customer->service_contact)) {
                $customers[$key][] = [$customer->id => "{$customer->subject} - {$customer->service_contact->name} "];

            } else {
                $customers[$key][] = [$customer->id => "{$customer->subject}"];

            }
        }
        // $customers = collect($customers);
        // $customers->prepend('Select a Subscription Service', '');

        $sla = ServiceLevel::pluck('name','id');

        // return $sla;

        // return \App\Modules\Subscription\Models\SubscriptionService::pluck('subject', 'id')->prepend('Select a Subscription Service', '');
        // return $customers;

        return view('ticketit::tickets.create', compact('priorities', 'categories', 'customers', 'sla'));
    }
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            ...
            'sla_id' => 'nullable|exists:service_level_aggrements,id',
            'subscription_service_id' => 'nullable',
        ...
        $ticket->sla_id = $request->sla_id;
        if(isset($request->subscription_service_id) && !empty($request->subscription_service_id)) {
            $ticket->subscription_service_id = $request->subscription_service_id;
        }
        ...
        $message = "The ticket was created by " . auth()->user()->name . ". Agent: " . $ticket->agent->name;

        $ticket->activities()->create(['title' => 'Created', 'owner_id' => 1, 'message' => $message, 'status' => $ticket->status->name]);
    ...
    public function update(Request $request, $id)
    {
        ...
        $message = "The ticket was updated by " . auth()->user()->name . ". Agent: " . $ticket->agent->name;

        $ticket->activities()->create(['title' => 'Updated', 'owner_id' => 1, 'message' => $message, 'status' => $ticket->status->name]);
